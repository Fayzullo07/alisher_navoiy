...
const GenreList = ({ search, devan_id, genre_id, setGenreId, firstFilter, auditory_age__in, text_type_id__in }) => {
    const [isInitialized, setIsInitialized] = useState(false);
    const [dataGenres, setDataGenres] = useState([]);

    const { data } = useQuery({
        queryKey: ["genre_list", devan_id.id, search, firstFilter.id, auditory_age__in, text_type_id__in],
        queryFn: async () => {
            return await devonsGetApi({ devan_id: devan_id.id, search, second: firstFilter.id == 0 ? "" : firstFilter.id, auditory_age__in: auditory_age__in ? auditory_age__in : "", text_type_id__in: text_type_id__in ? text_type_id__in : "" });
        }
    });
