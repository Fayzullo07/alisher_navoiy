...
const Filter = ({ text_types, auditory_ages }) => {
    return (
        <>
                    ...
                    <PopoverContent align="start">
                        <div className="">
                            <ScrollArea className="h-[15vh]">
                                ...
                                {auditory_ages.map((item: any, i: any) => (
                                    <div key={i} className="flex items-center space-x-2 hover:bg-blue-100 px-2 py-1 rounded-xl duration-300 cursor-pointer">
                                        <Checkbox id="terms" />
                                        <label
                                            htmlFor="terms"
...
const GazalList = ({ search, devan_id, genre_id, gazal_id, setGazal_id, firstFilter, current, setCurrent }) => {
    ...
    return (
        <>
            ...
            {firstFilter.id == 0 && (
                <div className="bg-white h-fit rounded-2xl text-center py-2 px-3" >
                    ...
                    <div className="flex items-center float-center gap-2  md:hidden mb-2">
                        {/* Filter mobile */}
                        <Filter text_types={data?.data?.text_types || []} auditory_ages={data?.data?.auditory_ages || []} />
                    </div>
                        ...
                        {data?.data?.main?.results.map((item: any, i: any) => (
                            <div key={i} >
                                ...
                                <div className=" block lg:hidden">
                                    <GazalMobile item={gazal_id.id} setGazal_id={setGazal_id} current={current} setCurrent={setCurrent}>
                                        <div
            ...
            {firstFilter.id == 3 && (
                <div className="bg-white h-fit rounded-2xl text-center py-2 px-3" >
                    <div className="text-xl flex-grow text-center font-semibold py-1 pb-2">{firstFilter.name}</div>
                    <div className="flex items-center  gap-2  p-1 rounded-full bg-gray-50 border w-full  mb-2">
                        <SearchIcon strokeWidth={1} size={20} />
                        <input value={genre_detail_number} onChange={(e) => {
                            if (/[a-zA-Z]/.test(e.target.value)) {
                                return
                        ...
                        {data?.data?.main?.results.map((item: any, i: any) => (
                            <div key={i}>
                                ...
                                <div className=" block md:hidden">
                                    <GazalMobile item={item.id} setGazal_id={setGazal_id} current={current} setCurrent={setCurrent}>
                                        <div
                                            className={` ${gazal_id.index == i ? "bg-blue-100" : ""} w-full text-start flex justify-between items-center hover:bg-blue-100  border-b duration-300 py-1 px-2 cursor-pointer`}
                                            onClick={() => setGazal_id({ ...item, index: i })}
                                        >
